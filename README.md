...
1. Open `Key Repeater.dmg`
2. Drag **Key Repeater** into the **Applications** folder
3. Launch from Launchpad or Spotlight

### Option B — Drop the `.app` directly

Drag `Key Repeater.app` from this repository into `/Applications`.
...
mv "dist/Key Repeater.app" "Key Repeater.app"

# Build the DMG installer
dmgbuild -s dmg_settings.py "Key Repeater" "Key Repeater.dmg"
```
...
- `key_repeater.py` — the Tk app source
- `setup.py` — py2app configuration
- `dmg_settings.py` — dmgbuild configuration
- `resources/AppIcon.icns` — app icon
- `Key Repeater.app` — built standalone macOS app
- `Key Repeater.dmg` — drag-to-Applications installer
